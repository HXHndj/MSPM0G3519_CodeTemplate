...
void EPD_ShowChar(uint16_t x,uint16_t y,uint16_t chr,uint16_t size1,uint16_t color)
{
	...
	for(i=0;i<size2;i++)
	{
        ...
        {temp=asc2_4824[chr1][i];} //调用4824字体
		else return;
		for(m=0;m<8;m++)
		{
...
/*******************************************************************
 * 函数说明：在墨水屏上生成并显示固定尺寸(37x37)的二维码
 * 入口参数：
 * x, y  : 二维码最左上角的起始坐标
 * text  : 要编码的字符串 (注意：Version 4 LOW容错最大支持约78个纯英文字符)
 * color : 二维码的颜色 (BLACK 或 RED)
 * 返回值： 无
 *******************************************************************/
void EPD_ShowQRCode(uint16_t x, uint16_t y, const char* text, uint8_t color)
{
    // 固定使用 Version 4
    uint8_t qrcode[qrcodegen_BUFFER_LEN_FOR_VERSION(4)];
    uint8_t tempBuffer[qrcodegen_BUFFER_LEN_FOR_VERSION(4)];

    // 强制 minVersion 和 maxVersion 都为 4，锁定生成 33x33 的二维码
    bool ok = qrcodegen_encodeText(text, tempBuffer, qrcode, qrcodegen_Ecc_LOW,
                                   4, 4, qrcodegen_Mask_AUTO, true);
    if (ok) {
        int size = qrcodegen_getSize(qrcode); // 必然返回 33
        int margin = 2; // 四周预留2像素白边，总尺寸变为 37x37
        ...
        for (int j = -margin; j < size + margin; j++) {
            for (int i = -margin; i < size + margin; i++) {
                ...
                if (i >= 0 && i < size && j >= 0 && j < size) {
                    isDark = qrcodegen_getModule(qrcode, i, j);
                }
                
                // 性能优化：因为背景已由 Paint_Clear(WHITE) 涂白，
                // 所以我们只挑出黑点进行绘制，忽略白点，大幅节省 MCU 算力
                if (isDark) {
                    Paint_SetPixel(x + (i + margin), y + (j + margin), color);
                }
            }
        }
    }
