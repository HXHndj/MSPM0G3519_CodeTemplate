/***************************************************************************************
  ...
  ***************************************************************************************
  * 修改说明：
  * 适配MSPM0G51X单片机，使用硬件SPI替换软件模拟SPI
  * 修改时间：2026.01.13
  */
...
void OLED_W_RES(uint8_t BitValue)
{
	...
	if (BitValue == 0){DL_GPIO_clearPins(OLED_RES_PORT, OLED_RES_PIN);}
	else if (BitValue == 1){DL_GPIO_setPins(OLED_RES_PORT, OLED_RES_PIN);}
}
...
void OLED_W_DC(uint8_t BitValue)
{
	...
	if (BitValue == 0){DL_GPIO_clearPins(OLED_DC_PORT, OLED_DC_PIN);}
	else if (BitValue == 1){DL_GPIO_setPins(OLED_DC_PORT, OLED_DC_PIN);}
}
...
void OLED_W_CS(uint8_t BitValue)
{
	...
	if (BitValue == 0){DL_GPIO_clearPins(OLED_CS_PORT, OLED_CS_PIN);}
	else if (BitValue == 1){DL_GPIO_setPins(OLED_CS_PORT, OLED_CS_PIN);}
}
...
void OLED_WriteCommand(uint8_t Command)
{
	...
	DL_SPI_transmitData8(SPI_0_INST, Command);
	while (DL_SPI_isBusy(SPI_0_INST));
...
/**
  * 函    数：OLED画线
  * 参    数：X0 指定线条的第一个点横坐标，范围：-32768~32767，屏幕区域：0~127
  * 参    数：Y0 指定线条的第一个点纵坐标，范围：-32768~32767，屏幕区域：0~63
  * 参    数：X1 指定线条的第二个点横坐标，范围：-32768~32767，屏幕区域：0~127
  * 参    数：Y1 指定线条的第二个点纵坐标，范围：-32768~32767，屏幕区域：0~63
  * 返 回 值：无
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
...
/**
  ...
  * 参    数：IsFilled 指定矩形是否填充，范围：OLED_UNFILLED / OLED_FILLED
  * 返 回 值：无
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
...
/**
  * 函    数：OLED画三角形
  * 参    数：X0 指定三角形第一个点的横坐标，范围：-32768~32767
  * 参    数：Y0 指定三角形第一个点的纵坐标，范围：-32768~32767
  * 参    数：X1 指定三角形第二个点的横坐标，范围：-32768~32767
  * 参    数：Y1 指定三角形第二个点的纵坐标，范围：-32768~32767
  * 参    数：X2 指定三角形第三个点的横坐标，范围：-32768~32767
  * 参    数：Y2 指定三角形第三个点的纵坐标，范围：-32768~32767
  * 参    数：IsFilled 指定三角形是否填充，范围：OLED_UNFILLED / OLED_FILLED
  * 返 回 值：无
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
...
/**
  * 函    数：OLED画圆
  * 参    数：X 指定圆心的横坐标，范围：-32768~32767，屏幕区域：0~127
  * 参    数：Y 指定圆心的纵坐标，范围：-32768~32767，屏幕区域：0~63
  * 参    数：Radius 指定圆的半径，范围：0~255
  * 参    数：IsFilled 指定圆是否填充，范围：OLED_UNFILLED / OLED_FILLED
  * 返 回 值：无
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
...
/**
  * 函    数：OLED画椭圆
  * 参    数：X 指定椭圆中心的横坐标，范围：-32768~32767
  * 参    数：Y 指定椭圆中心的纵坐标，范围：-32768~32767
  * 参    数：A 指定椭圆横向半轴长度，范围：0~255
  * 参    数：B 指定椭圆纵向半轴长度，范围：0~255
  * 参    数：IsFilled 指定椭圆是否填充，范围：OLED_UNFILLED / OLED_FILLED
  * 返 回 值：无
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_DrawEllipse(int16_t X, int16_t Y, uint8_t A, uint8_t B, uint8_t IsFilled)
{
	...
	{
		/*遍历椭圆的外围矩形，判断点是否在椭圆内部*/
		for (dy = Y - B; dy <= Y + B; dy ++)
		{
			for (dx = X - A; dx <= X + A; dx ++)
			{
				if ((int32_t)(dx - X) * (dx - X) * b2 + (int32_t)(dy - Y) * (dy - Y) * a2 <= a2 * b2)
				{
					OLED_DrawPoint(dx, dy);
				}
			}
...
/**
  * 函    数：OLED画圆弧
  * 参    数：X 指定圆弧圆心的横坐标，范围：-32768~32767
  * 参    数：Y 指定圆弧圆心的纵坐标，范围：-32768~32767
  * 参    数：Radius 指定圆弧的半径，范围：0~255
  * 参    数：StartAngle 指定圆弧的起始角度，范围：-180~180
  * 参    数：EndAngle 指定圆弧的终止角度，范围：-180~180
  * 参    数：IsFilled 指定圆弧是否填充，范围：OLED_UNFILLED / OLED_FILLED
  * 返 回 值：无
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_DrawArc(int16_t X, int16_t Y, uint8_t Radius, int16_t StartAngle, int16_t EndAngle, uint8_t IsFilled)
{
	...
	if (!IsFilled)		//指定圆弧不填充
	{
		...
		{
			/*计算当前点的角度*/
			int16_t angle = atan2(dy, dx) / 3.14 * 180;
			
			/*判断角度是否在指定范围内*/
			if (OLED_IsInAngle(dx, dy, StartAngle, EndAngle))
			{
			...
			if (OLED_IsInAngle(dy, dx, StartAngle, EndAngle))
			{
				OLED_DrawPoint(X + dy, Y + dx);
			}
			if (OLED_IsInAngle(-dy, dx, StartAngle, EndAngle))
			{
				OLED_DrawPoint(X - dy, Y + dx);
	...
	else				//指定圆弧填充（扇形填充）
	{
		...
		for (dy = Y - Radius; dy <= Y + Radius; dy ++)
		{
			for (dx = X - Radius; dx <= X + Radius; dx ++)
			{
				...
				{
					/*计算点的角度*/
					angle = atan2(dy - Y, dx - X) / 3.14 * 180;
					
					/*判断角度是否在指定范围内*/
					if (OLED_IsInAngle(dx - X, dy - Y, StartAngle, EndAngle))
					{
						OLED_DrawPoint(dx, dy);
					}
				}
